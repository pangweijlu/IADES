...
function [symbolic_data, pointers] =  timeseries2symbol0sigma(data, N, ...
    n, alphabet_size, NR_opt)   % re-named by AK
...
if nargin < 5
    % NR_opt = 2;   % blocked by AK
    NR_opt = 1;   % no numerosity reduction (default value) - added by AK
end
...
for i = 1 : length(data) - (N -1)                                       
    ...
    % MODIFIED BY AK - check for zero standard deviation
    if std(sub_section) >= eps   % AK: consider greater value
        
        % Z normalize it.
        sub_section = (sub_section - mean(sub_section))/std(sub_section);     
    
        % take care of the special case where there is no dimensionality
        % reduction 
        if N == n
            PAA = sub_section;
        ...
        % AK: assign the entire word to the middle-ranged alphabet
        current_string = ones(1, n)*ceil(alphabet_size/2);
    end
    % END OF MODIFICATION BY AK 
